...
int RealLand::costFee(int num) const {
	return event*cost_fee[num];
}

int Dorm::costFee(int num) const {
	return event*cost_fee[0];
}
...
Person::Person(int n, char* name)
	...
	penalty(0), feeFree(false), Odd(1), Even(1), jailFree(false), bankrupt(false) {            // 홀짝 기본 개수 0개
	this->name = new char[strlen(name) + 1];
...
void Person::moveForced(int num) {                                 // 월급 주는게 맞나 모르겠
	if (num < position) {
		money += SALARY, this->property += SALARY;
		cout << "월급을 획득합니다." << endl;
	}
	position = num;
}
...
{
	cout << "계속하려면 3을 입력하세요." << endl;
	int key = getNumber(3, 3);
	system("cls");
...
int isMonopoly(vector<Land*>& l) {
	...
	for (int i = 0; i<MAXLAND; i++) { // 수원 독점
		if (l[i]->getMonopoly() == Suwon && l[i]->owner() == NULL) {
			flag = false;
			break;
		}
		if (l[i]->getMonopoly() == Suwon && l[i]->owner() != NULL) {
	...
	for (int i = 0; i<MAXLAND; i++) { // 서울 독점
		if (l[i]->getMonopoly() == Seoul && l[i]->owner() == NULL) {
			flag = false;
			break;
		}
		if (l[i]->getMonopoly() == Seoul && l[i]->owner() != NULL) {
	...
	for (int i = 0; i<MAXLAND; i++) { // 기숙사 독점
		if (l[i]->getMonopoly() == dorm && l[i]->owner() == NULL) {
			flag = false;
			break;
		}
		if (l[i]->getMonopoly() == dorm && l[i]->owner() != NULL) {
